...
import (
	...
	"github.com/Hmac512/accumulator/pkg/accumulator"
	"github.com/Hmac512/accumulator/pkg/core/curves"
)
...
func main() {
	...
	fmt.Println("generates secret key and public key pair...")
	fmt.Printf("publishes public key %v\n", hex.EncodeToString(pkBytes))
	fmt.Printf("retains secret key %v\n", hex.EncodeToString(skBytes))

	// Enter initial elements
	fmt.Println("starts initiating an accumulator")
	var userInput []string
	scanner := bufio.NewScanner(os.Stdin)
	fmt.Println("enters the elements accumulated in the initial accumulator, press enter to start")
	for scanner.Scan() {
	...
	fmt.Println("issues a membership witness to a user")
	fmt.Println("enters the element that a membership witness is associated with")
	...
	fmt.Printf("issued membership witness associated to element %v\n", witnessInput)
	...
	fmt.Println("could batch update the accumulator")
	fmt.Println("That is, we can add/delete multiple elements together")
	var addInput []string
