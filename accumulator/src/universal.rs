...
//! use accumulator::setup::{Keypair, SetupParams};
//! use accumulator::positive::Accumulator;
//! use accumulator::universal::UniversalAccumulator;
//! use accumulator::persistence::{State, InitialElementsStore};
//!
...
use crypto_utils::msm::multiply_field_elems_with_same_group_elem;

use crypto_utils::serde_utils::*;
...
pub mod tests {
    ...
    type Fr254 = <Bn254 as Pairing>::ScalarField;
    ...
    /// Setup a universal accumulator, its keys, params and state for testing.
    pub fn setup_universal_accum_bn254(
        rng: &mut StdRng,
        max: u64,
    ) -> (
        SetupParams<Bn254>,
        Keypair<Bn254>,
        UniversalAccumulator<Bn254>,
        InMemoryInitialElements<Fr254>,
        InMemoryState<Fr254>,
    ) {
    ...
    #[test]
    fn batch_update_and_membership_bn254() {
        // Tests batch updates and batch membership witness generation
        let max = 100;
        let mut rng = StdRng::seed_from_u64(0u64);

        let (params, keypair, mut accumulator_1, initial_elements, mut state_1) =
            setup_universal_accum_bn254(&mut rng, max);

        // Create more accumulators to compare. Same elements will be added and removed from them as accumulator_1
        let mut accumulator_2: UniversalAccumulator<Bn254> = accumulator_1.clone();
        let mut state_2 = InMemoryState::<Fr254>::new();

        let mut accumulator_3: UniversalAccumulator<Bn254> = accumulator_1.clone();
        let mut state_3 = InMemoryState::<Fr254>::new();

        let additions: Vec<Fr254> = (0..10).map(|_| Fr254::rand(&mut rng)).collect();
        let removals: Vec<Fr254> = vec![0, 1, 6, 9].into_iter().map(|i| additions[i]).collect();
        ...
        let members = state_3.db.iter().copied().collect::<Vec<_>>();
        let d =
            UniversalAccumulator::<Bn254>::compute_d_for_batch_given_members(&removals, &members);
        assert_eq!(
            accumulator_3
                .compute_non_membership_witness_for_batch_given_d(
                    d,
